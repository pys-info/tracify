...
class ChannelTransformer:
    ...
    def __init__(self):
        """
            ...
            """
        self.channels = {}
        for channel_name, channel_config in app_settings.ISSUE_TRACKER_CHANNELS_CONFIGURATION.items():

            # Extract channel class and credentials from configuration
            channel_class_path, channel_credentials = channel_config["class"], channel_config["credentials"]

            # Dynamically import the channel class
            module_name, class_name = channel_class_path.rsplit(".", 1)
